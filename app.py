...
if uploaded_file is not None:
    ...
    if st.sidebar.button("Show Analysis"):
        ...
        fig, ax = plt.subplots()
        plt.plot(daily_timeline['Date'], daily_timeline['message'], color='magenta')
        plt.xticks(rotation='vertical', color='orange')
        st.pyplot(fig)
        ...
        st.title("Heatmap of weekly activity")
        ...
        # Most used words
        most_used_df = helper.most_used_words(selected_user, df)
        fig, ax = plt.subplots()
        ax.barh(most_used_df[0], most_used_df[1])
        plt.xticks(rotation='vertical')
